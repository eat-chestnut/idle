...
本轮以当前真实仓库为准，没有继续扩客户端业务，也没有补 backend 新接口。
真实收口点只有两类：

- 脚本工程质量：把 merge gate 和 online smoke 的执行入口、阶段边界、注释与 review 可读性继续收紧
- 交接材料质量：把 README 与合并前检查清单补到“团队能接手复跑、能区分历史快照和本轮复核”的水平

这意味着本轮的目标不是“再证明功能已经新增”，而是确保现有功能已经达到可交接、可维护、可做最终合并检查准备的标准。
...
本 README 负责交接说明，不负责替代本轮验证记录。
如果要判断“这次收口之后到底有没有真的跑过、哪些项通过了、哪些项还没完成”，统一看：

- `client/合并前检查清单.md` 里的“本轮复核记录”
- `client/合并前检查清单.md` 里的“历史快照（仅供参考）”

使用口径：

- “本轮复核记录”只写本次真实执行结果
- “历史快照”只用于团队交接，不替代本轮结论
- 若本轮没有重跑 GUI，必须明确写“沿用历史 GUI 基线”
