...
- `user://phase_one_local_save.json`
  - 作为正式单机存档对象，承接角色、主线、背包、穿戴、最近一次出战 / 结算承接结果，以及必要 UI 焦点

当前启动到进局的正式顺序是：

- 先做一次启动检查
- 再判断 `user://phase_one_local_save.json` 是否存在有效正式存档
- 有效则按“继续游戏”恢复 `runtime state`
- 无效则按“新开局”初始化默认正式存档，再进入本地主循环

当前关系是：

- 页面运行期继续优先读本地 `runtime state`
- `runtime state` 启动时会优先从 `user://phase_one_local_save.json` 恢复
- 当前入口页、角色/主线路径、背包与穿戴焦点会跟随正式存档恢复
- `battle_context_id`、最近一次 prepare / settle 摘要允许恢复，但 Battle 页不会伪造战斗进行中的瞬时表现
